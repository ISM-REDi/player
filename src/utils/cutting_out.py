...
def save_parquet(df, save_dir, filelabel, cond):
  df.to_parquet(save_dir+filelabel+'_dataset_'+cond+'.parquet')
...
def main(argv=None):
  ...
  parser.add_argument(
    ...
    dest='label',
    required=True,
    help='new dataset label')

  known_args, rest_args = parser.parse_known_args(argv)

  dataset_df = pd.read_parquet(known_args.dataset)
  detail_df = pd.read_parquet(known_args.detail)
  print(dataset_df.head(2))
  print(detail_df.head(2))

  full_df = pd.merge(dataset_df, detail_df, on='corpusId')
  # print(full_df.columns.values)
  # ['event_timestamp' 'corpusId' 'embedding' 'parentId' 'created' 'authors'
  # 'author_group_id' 'author_ids' 'author_names' 'edges' 'edge_count'
  #  'weights' 'paperId' 'DOI' 'title' 'year' 'journal' 'publicationTypes'
  #  'abstract' 'tldr' 'fields' 's2FieldsOfStudy']

  full_df['s2FieldsStr'] = full_df['s2FieldsOfStudy'].apply(lambda x: '+'.join(x))

  # conditions.
  # i.g.
  # [full_df['year'] >= 2020]
  # [full_df['s2FieldsStr'].str.contains('Mathematics', na=False)]
  cut_df = full_df.copy()
  cut_df = cut_df[cut_df['s2FieldsStr'].str.contains('Mathematics', na=False)]
  cut_df = cut_df[cut_df['year'] >= 2020]
  cut_df.drop(columns='s2FieldsStr', inplace=True)
  cut_df.reset_index(inplace=True)
  save_parquet(cut_df, known_args.savedir, known_args.label, 'in_s2-Math_ge_year-2020')
